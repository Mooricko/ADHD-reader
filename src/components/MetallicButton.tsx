...
export function MetallicButton({
  ...
  // Compute active label & icon if used as Play button
  const computedLabel =
    isPlaying !== undefined && label === "Get Started"
      ? isPlaying
        ? "Pause"
        : "Play"
      : label;
  ...
  const renderIcon = () => {
    ...
    if (isPlaying !== undefined) {
      return isPlaying ? (
        <Pause className="size-4.5 fill-white text-white drop-shadow-[0px_1px_2px_rgba(0,0,0,0.5)]" />
      ) : (
        <Play className="size-4.5 fill-white text-white ml-0.5 drop-shadow-[0px_1px_2px_rgba(0,0,0,0.5)]" />
      );
    }
    return (
      <Sparkles
  ...
  return (
    <div className={`relative inline-flex flex-col items-center justify-center ${className}`}>
        ...
        <div className={`relative transform-3d ${shellSize}`}>
          ...
          >
            {isIcon ? (
              ...
              <span
                className={`flex items-center gap-1.5 text-sm font-medium whitespace-nowrap text-white [text-shadow:0px_1px_2px_rgba(0,0,0,0.5)] ${SETTLE}`}
              >
                {isPlaying !== undefined && (
                  isPlaying ? (
      ...
      {showShortcutHint && (
        <span className="text-[10px] uppercase font-mono tracking-widest text-neutral-400 select-none opacity-60 hover:opacity-100 transition-opacity mt-1">
          Space
        </span>
      )}
    </div>
